...
class JdbcMediaItemRepositoryTest {
    ...
    void findById_returnsEmpty_whenNotFound() {
        Optional<MediaItem> opt = mediaItemRepository.findById(99999);
    ...
    void findByType_returnsEmpty_whenNoMatch() {
        List<MediaItem> items = mediaItemRepository.findByType("NONEXISTENT");
        assertTrue(items.isEmpty());
    }

    @Test
    void findByTitleContaining_returnsEmpty_whenNoMatch() {
        List<MediaItem> items = mediaItemRepository.findByTitleContaining("notfound");
        assertTrue(items.isEmpty());
    }

    @Test
    void findByAuthorContaining_returnsEmpty_whenNoMatch() {
        List<MediaItem> items = mediaItemRepository.findByAuthorContaining("notfound");
        assertTrue(items.isEmpty());
    }
    ...
    void deleteById_returnsFalse_whenNotFound() {
        boolean deleted = mediaItemRepository.deleteById(99999);
        assertFalse(deleted);
    }
    ...
    void update_throws_whenIdNotExists() {
        ...
        assertThrows(RuntimeException.class, () -> mediaItemRepository.update(item));
    }

    @Test
    void updateAvailableCopies_throws_whenNotFound() {
        assertThrows(com.example.library.repository.DataAccessException.class, () -> mediaItemRepository.updateAvailableCopies(99999, 5));
    }

    @Test
    void search_returnsEmpty_whenNoMatch() {
        List<MediaItem> items = mediaItemRepository.search("notfound");
        assertTrue(items.isEmpty());
    }
}
