...
class JdbcLoanRepositoryTest {
    ...
    void findById_returnsEmpty_whenNotFound() {
        Optional<Loan> opt = loanRepository.findById(99999);
        assertFalse(opt.isPresent());
    }

    @Test
    void findAll_returnsEmpty_whenNoLoans() {
        // Clean DB to ensure empty
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("DELETE FROM loan")) {
            pstmt.executeUpdate();
        } catch (SQLException e) {
            fail("Failed to clean loans");
        }
    ...
    @Test
    void findByUserId_returnsEmpty_whenNoLoansForUser() {
    ...
    void findByStatus_returnsEmpty_whenNoMatch() {
        List<Loan> loans = loanRepository.findByStatus("NONEXISTENT");
        assertTrue(loans.isEmpty());
    }
    ...
    void update_throws_whenIdNotExists() {
        Loan loan = new Loan();
        loan.setLoanId(99999);
        loan.setUserId(testUserId);
        loan.setLoanDate(LocalDate.now());
        loan.setDueDate(LocalDate.now().plusWeeks(2));
        loan.setStatus("ACTIVE");
        assertThrows(RuntimeException.class, () -> loanRepository.update(loan));
    }

    @Test
    void deleteById_doesNothing_whenNotFound() {
        assertDoesNotThrow(() -> loanRepository.deleteById(99999));
    }
}
