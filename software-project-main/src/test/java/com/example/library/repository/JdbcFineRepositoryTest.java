...
class JdbcFineRepositoryTest {
    ...
    void findById_returnsEmpty_whenNotFound() {
        Optional<Fine> opt = fineRepository.findById(99999);
        assertFalse(opt.isPresent());
    }

    @Test
    void findAll_returnsEmpty_whenNoFines() {
        // Clean DB to ensure empty
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement pstmt = conn.prepareStatement("DELETE FROM fine")) {
            pstmt.executeUpdate();
        } catch (SQLException e) {
            fail("Failed to clean fines");
        }
    ...
    @Test
    void findByUserId_returnsEmpty_whenNoFinesForUser() {
        List<Fine> fines = fineRepository.findByUserId(99999);
        assertTrue(fines.isEmpty());
    }

    @Test
    void findByLoanId_returnsEmpty_whenNoFinesForLoan() {
        Optional<Fine> opt = fineRepository.findByLoanId(99999);
        assertFalse(opt.isPresent());
    }

    @Test
    void findByStatus_returnsEmpty_whenNoMatch() {
        List<Fine> fines = fineRepository.findByStatus("NONEXISTENT");
        assertTrue(fines.isEmpty());
    }
    ...
    void update_throws_whenIdNotExists() {
        Fine fine = new Fine();
        fine.setFineId(99999);
        fine.setLoanId(testLoanId);
        fine.setAmount(BigDecimal.TEN);
        fine.setStatus("UNPAID");
        fine.setIssuedDate(LocalDate.now());
        assertThrows(com.example.library.repository.DataAccessException.class, () -> fineRepository.update(fine));
    }

    @Test
    void deleteById_doesNothing_whenNotFound() {
        assertDoesNotThrow(() -> fineRepository.deleteById(99999));
    }
}
