...
import com.example.library.notification.EmailNotifier;
...
public class AdminFrame extends JFrame {
    ...
    private JPanel createUserManagementPanel() {
        ...
        // Store table model and table for refresh and actions
        panel.putClientProperty("tableModel", tableModel);
        panel.putClientProperty("userTable", userTable);
    ...
    private void createNewUser() {
        ...
        saveButton.addActionListener(e -> {
            ...
            try {
                ...
                // Send welcome email (best-effort - failures are logged inside EmailNotifier)
                try {
                    EmailNotifier notifier = new EmailNotifier();
                    notifier.notify(savedUser,
                            "Welcome " + savedUser.getUsername() + "! Your library account has been created.");
                } catch (Exception ignored) {
                    // Email failures should not prevent user creation
                }
    ...
    private void editUser() {
        ...
        try {
            ...
            // Don't allow editing current admin user
            if (user.getUserId().intValue() == currentUser.getUserId()) {
                JOptionPane.showMessageDialog(this, "You cannot edit your own admin account",
                        "Error", JOptionPane.ERROR_MESSAGE);
                return;
            }
    ...
    private void deleteUser() {
        ...
        try {
            ...
            int confirm = JOptionPane.showConfirmDialog(this,
                    ...
                    "Role: " + role + "\n" +
                    "User ID: " + userId + "\n\n" +
                    "This action will permanently delete the user from the database.",
                    "Confirm Delete User", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
            
            if (confirm == JOptionPane.YES_OPTION) {
                // Check if user has active loans
                List<Loan> userLoans = libraryService.getUserLoans(userId);
                boolean hasActiveLoans = userLoans.stream()
                    .anyMatch(loan -> "ACTIVE".equals(loan.getStatus()));
                
                if (hasActiveLoans) {
                    JOptionPane.showMessageDialog(this,
                            "Cannot delete user: User has active loans.\n" +
                            "Please return all items before deleting the user.",
                            "Error", JOptionPane.ERROR_MESSAGE);
                    return;
                }
